...
class acusar_con_proceso(object):
    ...
    def get_json_solicitudes(self, registro, numero_fila, file_name, file_64):
        ...
        try:
            ...
            UtilLog.get_instance().write(f"get_json_solicitudes radicado_id {e}")
        try:
            fecha_radicacion = columnas[0][1]  # 20189999992018999999
        except Exception as e:
            UtilLog.get_instance().write(f"get_json_solicitudes fecha_radicacion {e}")
        try:
            folios_dependiente = columnas[0][2]  # MEDELLIN
        except Exception as e:
            UtilLog.get_instance().write(f"get_json_solicitudes folios_dependiente {e}")
        try:
            costo_impresion = columnas[0][3]  # CIVIL CIRCUITO No. 2
        except Exception as e:
            UtilLog.get_instance().write(f"get_json_solicitudes costo_impresion {e}")
